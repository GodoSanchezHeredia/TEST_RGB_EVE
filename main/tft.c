...
void TFT_init(void)
{
    int ret = 0;
    ret = EVE_init();
    ESP_LOGI(TAG,"return init EVE: %d", ret);

    if (ret != E_OK){
        ESP_LOGE(TAG, "FAILD INIT EVE");
        return;
    }
...
/* we are running on a 32 bit controller and might be even using DMA, no need to optimize to the max */
